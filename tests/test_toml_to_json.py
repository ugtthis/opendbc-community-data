...
x_names = []
      ...
      self.assertEqual(payload["_metadata"]["total"], "1 cars")
      ...
      self.assertEqual(car["discord_names"], ["example-user"])
      self.assertEqual(car["x_names"], [])
...
branch_desc = "Missing required hardware"
      ...
      with self.assertRaisesRegex(
        ValueError,
        "missing required field\\(s\\): hardware_needed, branch_name, branch_url, wiki_url, discord_url, discord_names, x_names",
      ):
        build_wip_json(wip_dir, output_path)
...
x_names = []
      ...
      with self.assertRaisesRegex(
        ValueError,
        "at least one contributor is required in discord_names or x_names",
      ):
        build_wip_json(wip_dir, output_path)
